...
Semantics of an expression is defined only where it differs from, or is
not defined in link:expression_concept.html#matrix_expression[Matrix
Expression] .
...
Inserts an element at `m (i, j)` with value `t`. The storage requirement
of the Matrix may be increased.
...
`value_type ()`. The storage requirement of the Matrix may be decreased.
